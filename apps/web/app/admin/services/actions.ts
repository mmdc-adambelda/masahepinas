...
export async function toggleServiceCategoryActive(categoryId: string, isActive: boolean): Promise<ServiceCategoryResult> {
  await requireSuperadmin();
  const supabase = await createSupabaseServerClient();

  const { error } = await supabase.from('service_categories').update({ is_active: isActive }).eq('id', categoryId);
